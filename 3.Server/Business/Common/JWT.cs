...
namespace Business.Common
{
        ...
        public static string Decode(string token, byte[] key, bool verify = true)
        {
            ...
            if (verify)
            {
                ...
                if (payloadData.ContainsKey("exp") && payloadData["exp"] != null)
                {
                    ...
                    {
                        throw new ExpiredAuthenticationException(-500, "Token has expired");
                    }
                }
            }
    ...
    {
        public ExpiredAuthenticationException(int errorCode, string message)
            : base(message)
        {
        }
    }
    ...
    {
        public static string GetToken(object payload, string key = null)
        {
            string secretKey = key == null ? AuthorizationConstants.JWT_SECRET_KEY : key;
            string token = JsonWebToken.Encode(payload, secretKey, JwtHashAlgorithm.HS256);
            return token;
        }

        public static T VerifyToken<T>(string token, string key = null)
        {
            string secretKey = key == null ? AuthorizationConstants.JWT_SECRET_KEY : key;
            String jsonPayload;
            try
